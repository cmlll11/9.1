...
class RidgeProbe:
    ...
    @classmethod
    def fit(cls, features: np.ndarray, labels: np.ndarray, *, alpha: float = 1.0) -> "RidgeProbe":
        ...
        return cls(mean, std, parameters[:-1], float(parameters[-1]), float(alpha))
    ...
    def save(self, path: str | Path) -> None:
        ...
        np.savez(output, feature_mean=self.feature_mean, feature_std=self.feature_std, weight=self.weight,
                 bias=np.asarray(self.bias), alpha=np.asarray(self.alpha), feature_names=np.asarray(FEATURE_NAMES))
    ...
    def load(cls, path: str | Path) -> "RidgeProbe":
        ...
        if names != FEATURE_NAMES:
            raise ValueError("probe feature schema does not match the current protocol")
        return cls(archive["feature_mean"], archive["feature_std"], archive["weight"], float(archive["bias"]), float(archive["alpha"]))
